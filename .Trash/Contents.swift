		...
		tree1.run(SKAction.move(to: CGPoint(x: 120, y: tree1.frame.height/2), duration: 1), completion: {
			...
			tree2.run(SKAction.move(to: CGPoint(x: self.frame.width/2, y: tree2.frame.height/2), duration: 1), completion: {
				...
				tree3.run(SKAction.move(to: CGPoint(x: self.frame.width*0.75, y: tree3.frame.height/2), duration: 1), completion: {
					...
					let swipeArrow = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "NextArrow")))
					swipeArrow.setScale(0.25)
					swipeArrow.position = CGPoint(x: self.frame.width/2, y: self.frame.height/2)
					swipeArrow.alpha = 0;
					self.addChild(swipeArrow)
					
					let continueText = SKLabelNode(text: "Swipe to continue")
					continueText.fontSize = 45
					continueText.color = #colorLiteral(red: 0.3411357999, green: 0.3411998153, blue: 0.341131717, alpha: 1)
					continueText.alpha = 0
					continueText.position = CGPoint(x: self.size.width * 0.5, y: swipeArrow.position.y-continueText.frame.height-25) //Place ourselves 25 below the bototm of the arrow
					...
					titleText.alpha = 0
					print(self.frame.height-titleText.frame.height-50)
					titleText.position = CGPoint(x: self.size.width * 0.5, y: self.frame.height-titleText.frame.height-50) //Place ourselves 25 below the bototm of the arrow
					self.addChild(titleText)
					
					swipeArrow.run(SKAction.fadeIn(withDuration: 0.5))
					continueText.run(SKAction.fadeIn(withDuration: 0.9))
					titleText.run(SKAction.fadeIn(withDuration: 0.5), completion: {
						//We've presented everything, setup the gesture to dismiss/move on
						let swipeRecognizer:UISwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action:#selector(self.finishScene(swipe:)))
